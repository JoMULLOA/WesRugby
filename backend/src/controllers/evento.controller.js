...
async function fetchParticipacionesEventoData({ id, categorias }) {
  const participacionRepository =
    AppDataSource.getRepository(ParticipacionEvento);
  ...
  const ParticipacionEventoDeportivo = (
    await import("../entity/participacionEventoDeportivo.entity.js")
  ).default;
  const EventoDeportivo = (await import("../entity/eventoDeportivo.entity.js"))
    .default;
  const participacionDeportivaRepository = dataSource.getRepository(
  ...
  if (esUUID(id)) {
    ...
    if (!Number.isNaN(eventoIdInt)) {
      ...
      eventoInfo = await eventoRepository.findOne({
        where: { id: eventoIdInt },
      });
    }
  }
  ...
  if (typeof categorias === "string" && categorias.trim().length > 0) {
    ...
    if (parsedCategorias.length > 0) {
      categoriasFiltroSet = new Set(
        parsedCategorias.map((cat) => cat.toLowerCase()),
      );
    }
  }
  ...
  for (const participacion of participaciones) {
    const categoriaOriginal = (
      participacion.categoria || "sin categoria"
    ).trim();
  ...
  const estadisticasPorRama = Array.from(
    estadisticasPorRamaMap.values(),
  ).filter((registro) => registro.totalRama > 0);
...
export async function obtenerEventosDisponibles(req, res) {
  try {
    ...
    const EventoDeportivo = (
      await import("../entity/eventoDeportivo.entity.js")
    ).default;
    const eventoDeportivoRepository = dataSource.getRepository(EventoDeportivo);
    ...
    const todosLosEventos = [
      ...eventosRegularesActivos,
      ...eventosDeportivosFormateados,
    ];
    ...
    handleSuccess(
      res,
      200,
      "Eventos disponibles obtenidos exitosamente",
      todosLosEventos,
    );
  } catch (error) {
...
export async function editarParticipacion(req, res) {
  try {
    ...
    const tiempoTranscurrido =
      Date.now() - new Date(participacion.createdAt).getTime();
    ...
    const participantesNormalizados =
      normalizeParticipantsInput(listaInvitados);
    ...
    participacion.listaInvitados = formatParticipantsList(
      participantesNormalizados,
    );

    let participacionActualizada;
    if (esEventoDeportivo) {
      participacionActualizada =
        await participacionDeportivaRepository.save(participacion);
    } else {
      participacionActualizada =
        await participacionRepository.save(participacion);
    }

    handleSuccess(
      res,
      200,
      "Participación actualizada exitosamente",
      participacionActualizada,
    );
  } catch (error) {
...
export async function participarEnEvento(req, res) {
  try {
    ...
    if (esUUID(eventoId)) {
      ...
      if (
        eventoDeportivo &&
        (eventoDeportivo.estado === "programado" ||
          eventoDeportivo.estado === "activo")
      ) {
    ...
    if (evento.esEventoDeportivo) {
      ...
      if (eventoDeportivo && eventoDeportivo.categoria) {
        const categoriasPermitidas = eventoDeportivo.categoria
          .split(",")
          .map((cat) => cat.trim());
    ...
    if (evento.esEventoDeportivo) {
      ...
      const participacionExistente =
        await participacionDeportivaRepository.findOne({
      ...
      if (participacionExistente) {
        return handleErrorClient(
          ...
          400,
          "Error al participar",
          "Ya tienes una participación registrada en este evento para la categoría " +
            categoria,
        );
      }
      ...
      participacionGuardada =
        await participacionDeportivaRepository.save(nuevaParticipacion);
    } else {
      ...
      participacionGuardada =
        await participacionRepository.save(nuevaParticipacion);
    }
...
export async function obtenerMisParticipacionesEvento(req, res) {
  try {
    ...
    const participacionesDeportivas =
      await participacionDeportivaRepository.find({
      ...
      participacionesDeportivas.map(async (participacion) => {
        ...
        if (eventoDeportivo) {
          ...
          const eventoFormateado = {
            ...
            estado:
              eventoDeportivo.estado === "programado" ||
              eventoDeportivo.estado === "confirmado"
                ? "activo"
                : eventoDeportivo.estado,
            lugar: eventoDeportivo.lugar,
            categoria: eventoDeportivo.categoria,
            tipoEvento: eventoDeportivo.tipoEvento,
            esEventoDeportivo: true,
          ...
          eventosPorId[eventoDeportivo.id].totalNinos +=
            participacion.cantidadNinos;
          eventosPorId[eventoDeportivo.id].categorias.push(
    ...
    const todasLasParticipaciones = [
      ...participacionesRegularesConEventos,
      ...participacionesDeportivasConEventos,
    ];
...
export async function obtenerCategoriasRegistradas(req, res) {
  try {
    ...
    if (esUUID(eventoId)) {
      const { AppDataSource: dataSource } = await import(
        "../config/configDb.js"
      );
      const EventoDeportivo = (
        await import("../entity/eventoDeportivo.entity.js")
      ).default;
      const eventoDeportivoRepository =
        dataSource.getRepository(EventoDeportivo);
      ...
      if (eventoDeportivo && eventoDeportivo.categoria) {
        categoriasDelEvento = eventoDeportivo.categoria
          .split(",")
          .map((cat) => cat.trim());
      }
    }
    ...
    const todasCategorias = [
      ...new Set([...categoriasPredeterminadas, ...categoriasDelEvento]),
    ];

    const categoriasDisponibles = todasCategorias.filter(
      (cat) => !categoriasRegistradas.includes(cat),
    );
...
export async function obtenerParticipacionesEvento(req, res) {
  try {
    ...
    const { payload } = await fetchParticipacionesEventoData({
      id,
      categorias,
    });
    ...
    handleSuccess(
      res,
      200,
      "Participaciones del evento obtenidas exitosamente",
      payload,
    );
  } catch (error) {
...
export async function descargarParticipacionesEventoPdf(req, res) {
  try {
    const { id } = req.params;
    const { categorias } = req.query;
    const { evento, payload } = await fetchParticipacionesEventoData({
      id,
      categorias,
    });
