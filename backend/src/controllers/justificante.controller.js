...
export async function crearJustificanteApoderado(req, res) {
  try {
    ...
    if (!rutApoderado) return handleErrorClient(res, 401, "Rut apoderado no disponible");
    const { estudianteRut, fechaInicio, fechaFin, tipo, motivo, descripcion } = req.body;
...
export async function listarJustificantesApoderado(req, res) {
  try {
    ...
    const qb = justificanteRepository.createQueryBuilder("j").where("j.apoderadoRut = :rut", { rut: rutApoderado });
    ...
    handleSuccess(res, 200, "Justificantes apoderado", {
      ...
      paginacion: { total, pagina: page, limite: take, totalPaginas: Math.max(Math.ceil(total / take), 1) },
    });
  } catch (error) {
...
export async function listarJustificantesDirectiva(req, res) {
  try {
    const { pagina = 1, limite = 30, estado, estudianteRut, apoderadoRut, desde, hasta } = req.query;
    ...
    if (estudianteRut) qb.andWhere("j.estudianteRut = :estudianteRut", { estudianteRut });
    if (apoderadoRut) qb.andWhere("j.apoderadoRut = :apoderadoRut", { apoderadoRut });
    if (desde && hasta) {
      qb.andWhere("j.fechaInicio <= :hasta AND (j.fechaFin IS NULL OR j.fechaFin >= :desde)", { desde, hasta });
    } else if (desde) {
      qb.andWhere("(j.fechaFin IS NULL AND j.fechaInicio = :desde) OR (j.fechaFin IS NOT NULL AND j.fechaFin >= :desde)", { desde });
    }
    ...
    handleErrorServer(res, 500, "Error listando justificantes directiva", error.message);
  }
}
...
export async function actualizarEstadoJustificante(req, res) {
  try {
    ...
    const justificante = await justificanteRepository.findOne({ where: { id } });
    if (!justificante) return handleErrorClient(res, 404, "Justificante no encontrado");
    const allowed = ["pendiente", "aprobado", "rechazado", "observado"];
    if (estado && !allowed.includes(String(estado))) return handleErrorClient(res, 400, "Estado inválido");
    const nuevoEstado = estado ? String(estado) : justificante.estado;
    justificante.estado = nuevoEstado;
    justificante.motivoRechazo = nuevoEstado === "rechazado" ? (motivoRechazo || null) : null;
    justificante.observacionesDirectiva = observacionesDirectiva || justificante.observacionesDirectiva || null;
    justificante.revisadoPorRut = req.user?.rut || justificante.revisadoPorRut || null;
    ...
    if (nuevoEstado === "aprobado") {
      try {
        ...
        const registroRepo = AppDataSource.getRepository(RegistroAsistenciaSchema);
        const sesiones = await sesionRepo
          .createQueryBuilder("sesion")
          .where("sesion.fecha BETWEEN :fi AND :ff", { fi: fechaInicio, ff: fechaFin })
          .getMany();
        if (sesiones.length) {
        ...
        console.error("⚠️ Error propagando justificación a registros asistencia:", e.message);
      }
    }
...
export async function actualizarMesesExencion(req, res) {
  try {
    ...
    if (justificante.estado !== "aprobado") {
      return handleErrorClient(res, 409, "Solo justificantes aprobados pueden tener meses de exención");
    }
    if (!Array.isArray(meses)) return handleErrorClient(res, 400, "meses debe ser un array");
    ...
    const mesesRango = [];
    let cursor = new Date(fi.getFullYear(), fi.getMonth(), 1);
    ...
    const saved = await justificanteRepository.save(justificante);
    handleSuccess(res, 200, "Meses de exención actualizados", mapJustificante(saved));
  } catch (error) {
    handleErrorServer(res, 500, "Error actualizando meses exención", error.message);
  }
}
...
export async function obtenerJustificadosPorFecha(req, res) {
  try {
    ...
    if (isNaN(fechaISO.getTime())) return handleErrorClient(res, 400, "Fecha inválida");
    const listaRuts = typeof ruts === "string" ? ruts.split(",").map(r => r.trim()).filter(Boolean) : [];
    if (!listaRuts.length) return handleErrorClient(res, 400, "RUTs requeridos");
    const justificantes = await justificanteRepository
      ...
      .andWhere("j.fechaInicio <= :fecha AND (j.fechaFin IS NULL OR j.fechaFin >= :fecha)", { fecha })
      .getMany();
    const agrupados = listaRuts.map(rut => ({
      estudianteRut: rut,
      justificantes: justificantes.filter(j => j.estudianteRut === rut).map(mapJustificante),
    }));
    ...
    handleErrorServer(res, 500, "Error obteniendo justificantes por fecha", error.message);
  }
}
