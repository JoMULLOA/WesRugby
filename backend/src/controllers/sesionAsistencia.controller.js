  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "Datos incompletos. Se requiere nombre, fecha, categoria y asistencias.",
      });
    }
    ...
    const [estudiantes, estudiantesError] =
      await getEstudiantesByRutListService(rutEstudiantes);

    if (estudiantesError) {
      console.warn(
        "⚠️ Error al obtener nombres de estudiantes, usando datos del frontend",
      );
    }
    ...
    const asistenciasEnriquecidas = asistencias.map((asistencia) => {
      const estudiante = estudiantes?.find(
        (e) => e.rut === asistencia.rutEstudiante,
      );
      return {
        ...asistencia,
        nombreEstudiante:
          estudiante?.nombre || `Estudiante ${asistencia.rutEstudiante}`,
      };
    });
    ...
    console.log(
      "✅ Sesión de asistencia creada exitosamente:",
      sesionCreada.id,
    );
...
export const getMisSesiones = async (req, res) => {
  try {
    ...
    const [sesiones, error] = await getSesionesByEntrenadorService(
      userAuth.rut,
      limite,
    );
...
export const getSesionDetalle = async (req, res) => {
  try {
    ...
    const [sesionConRegistros, error] = await getSesionConRegistrosService(
      parseInt(sesionId),
    );
    ...
    console.log(
      "✅ Sesión encontrada con",
      sesionConRegistros.registros.length,
      "registros",
    );
...
export const getAllSesiones = async (req, res) => {
  try {
    ...
    if (userAuth.rol === "directiva" || userAuth.rol === "apoderado") {
      ...
      [sesiones, error] = await getSesionesByEntrenadorService(
        userAuth.rut,
        limite,
      );
    } else {
