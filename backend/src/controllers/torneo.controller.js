...
const participacionRepository =
  AppDataSource.getRepository(ParticipacionTorneo);
  ...
  try {
    ...
    if (!nombre || !fechaTorneo || !categorias || !Array.isArray(categorias)) {
      return handleErrorClient(
        res,
        400,
        "Datos incompletos: nombre, fechaTorneo y categorias son requeridos",
      );
    }
    ...
    const categoriasInvalidas = categorias.filter(
      (cat) => !categoriasPermitidas.includes(cat),
    );
    if (categoriasInvalidas.length > 0) {
      return handleErrorClient(
        res,
        400,
        `Categorías inválidas: ${categoriasInvalidas.join(", ")}`,
      );
    }
    ...
    console.log(
      `✅ Torneo creado por directiva ${req.user.rut}: ${torneoGuardado.nombre}`,
    );
...
export const obtenerTorneos = async (req, res) => {
  try {
    ...
    if (
      estado &&
      ["abierto", "cerrado", "finalizado", "cancelado"].includes(estado)
    ) {
    ...
    const torneosConEstadisticas = torneos.map((torneo) => {
      ...
      torneo.participaciones.forEach((participacion) => {
        ...
        participacionesPorCategoria[participacion.categoria].ninos +=
          participacion.cantidadNinos;
        participacionesPorCategoria[participacion.categoria].invitados +=
          participacion.cantidadInvitados;
...
export const obtenerTorneosDisponibles = async (req, res) => {
  try {
    ...
    handleSuccess(
      res,
      200,
      "Torneos disponibles obtenidos exitosamente",
      torneosDisponibles,
    );
  } catch (error) {
  ...
  try {
    ...
    if (!torneoId || !nombreRama || !categoria || cantidadNinos === undefined) {
      return handleErrorClient(
        res,
        400,
        "Datos incompletos: torneoId, nombreRama, categoria y cantidadNinos son requeridos",
      );
    }
    ...
    if (torneo.estado !== "abierto") {
      return handleErrorClient(
        res,
        400,
        "El torneo no está disponible para participación",
      );
    }
    ...
    if (!torneo.categorias.includes(categoria)) {
      return handleErrorClient(
        res,
        400,
        "La categoría seleccionada no está disponible en este torneo",
      );
    }
    ...
    if (participacionExistente) {
      return handleErrorClient(
        res,
        400,
        "Ya tienes una participación registrada en esta categoría para este torneo",
      );
    }
    ...
    const participacionGuardada =
      await participacionRepository.save(nuevaParticipacion);

    console.log(
      `✅ Participación registrada: ${req.user.rut} en torneo ${torneoId}, categoría ${categoria}`,
    );
...
export const actualizarEstadoTorneo = async (req, res) => {
  try {
    ...
    console.log(
      `✅ Estado del torneo ${id} actualizado a ${estado} por directiva ${req.user.rut}`,
    );
