...
import { getNoticiasService } from "../services/noticia.service.js";
import { getAuspiciadoresService } from "../services/auspiciador.service.js";
import { getMerchandisingService } from "../services/merchandising.service.js";
import {
  handleErrorClient,
  handleErrorServer,
  handleSuccess,
  ...
  try {
    ...
    if (noticiasError) {
      return handleErrorClient(
        res,
        404,
        "Error al obtener noticias: " + noticiasError,
      );
    }
    ...
    if (auspiciadoresError) {
      return handleErrorClient(
        res,
        404,
        "Error al obtener auspiciadores: " + auspiciadoresError,
      );
    }
    ...
    if (merchandisingError) {
      return handleErrorClient(
        res,
        404,
        "Error al obtener merchandising: " + merchandisingError,
      );
    }
    ...
    handleSuccess(
      res,
      200,
      "Información pública obtenida exitosamente",
      response,
    );
  } catch (error) {
...
router.get("/club-info", (req, res) => {
  try {
    handleSuccess(
      res,
      200,
      "Información del Club Wessex obtenida exitosamente",
      clubWessexInfo,
    );
  } catch (error) {
    handleErrorServer(res, 500, error.message);
