...
export const vehiculoBodyValidation = Joi.object({
  patente: Joi.string()
    ...
    .messages({
      ...
      "string.pattern.base":
        "Formato de patente inválido. Debe ser AA1234 o AAAA12.",
    }),
  tipo: Joi.string()
    .valid(
      "sedan",
    ...
    .messages({
      ...
      "any.only":
        "El tipo debe ser uno de: sedan, hatchback, suv, pickup, furgon, camioneta, coupe, convertible, otro.",
    }),
  marca: Joi.string().min(2).max(50).messages({
  ...
  modelo: Joi.string().min(2).max(50).messages({
  ...
  color: Joi.string().min(2).max(50).messages({
  ...
  nro_asientos: Joi.number().integer().min(2).max(9).messages({
  ...
  tipoCombustible: Joi.string()
    ...
    .messages({
      ...
      "any.only":
        "El tipo de combustible debe ser uno de: bencina, petroleo, electrico, hibrido, gas.",
    }),
  documentacion: Joi.string().min(5).max(500).messages({
