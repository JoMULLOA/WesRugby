...
export const viajeBodyValidation = Joi.object({
  ubicaciones: Joi.array()
    ...
    .messages({
      "array.length":
        "Debe proporcionar exactamente 2 ubicaciones: origen y destino",
    }),

  fechaHoraIda: Joi.date().min("now").required().messages({
    "date.min":
      "La fecha y hora de ida no puede ser anterior al momento actual",
  }),

  fechaHoraVuelta: Joi.date()
    .min(Joi.ref("fechaHoraIda"))
    .allow(null)
    .messages({
      "date.min":
        "La fecha y hora de vuelta no puede ser anterior a la fecha de ida",
    }),
  ...
  plazasDisponibles: Joi.number()
    .integer()
    .min(1)
    .max(Joi.ref("maxPasajeros"))
    .required(),
  ...
  .messages({
    ...
    "custom.idaVuelta":
      "Para viajes de ida y vuelta debe especificar fecha y hora de vuelta",
  });
...
export const cambioEstadoAutomaticoValidation = Joi.object({
  ...
  estadoAnterior: Joi.string()
    .valid("activo", "en_curso", "completado", "cancelado")
    .required(),
  estadoNuevo: Joi.string()
    .valid("activo", "en_curso", "completado", "cancelado")
    .required(),
