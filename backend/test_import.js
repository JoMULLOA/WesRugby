...
datosEjemplo.forEach((estudiante) => {
  const generateEmail = (nombreCompleto) => {
    ...
    const primerApellido =
      nombres[nombres.length - 2] || nombres[nombres.length - 1];
    return `${primerNombre}.${primerApellido}0@wessex.cl`;
  };

  console.log(
    `  👤 ${estudiante.nombreResponsable} → ${generateEmail(estudiante.nombreResponsable)}`,
  );
  if (estudiante.nombreResponsable2) {
    console.log(
      `  👤 ${estudiante.nombreResponsable2} → ${generateEmail(estudiante.nombreResponsable2)}`,
    );
  }
  console.log("");
});
...
console.log(
  "Los datos están listos para ser procesados por el controlador de importación.",
);
