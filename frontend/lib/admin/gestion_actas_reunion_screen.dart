...
class GestionActasReunionScreen extends StatefulWidget {
  ...
  @override
  State<GestionActasReunionScreen> createState() => _GestionActasReunionScreenState();
}
...
class _GestionActasReunionScreenState extends State<GestionActasReunionScreen> {
  ...
  Future<void> _cargarActas() async {
    ...
    try {
      ...
      if (response.success && response.data != null) {
        setState(() {
          _actas = List<Map<String, dynamic>>.from(response.data['data'] ?? response.data);
        });
      } else {
        setState(() {
  ...
  void _mostrarDialogoCrearEditar({Map<String, dynamic>? acta}) {
    ...
    final descripcionController = TextEditingController(text: acta?['descripcion'] ?? '');
    final lugarController = TextEditingController(text: acta?['lugar'] ?? '');
    final asistentesController = TextEditingController(text: acta?['asistentes'] ?? '');
    final acuerdosController = TextEditingController(text: acta?['acuerdos'] ?? '');
    final compromisosController = TextEditingController(text: acta?['proximosCompromiso'] ?? '');
    
    DateTime fechaSeleccionada = acta != null ? DateTime.parse(acta['fecha']) : DateTime.now();
    TimeOfDay? horaInicio = acta?['horaInicio'] != null ? 
        TimeOfDay.fromDateTime(DateFormat('HH:mm:ss').parse(acta!['horaInicio'])) : null;
    TimeOfDay? horaFin = acta?['horaFin'] != null ? 
        TimeOfDay.fromDateTime(DateFormat('HH:mm:ss').parse(acta!['horaFin'])) : null;

    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(isEditing ? 'Editar Acta de Reunión' : 'Nueva Acta de Reunión'),
                    ...
                    Expanded(
                      child: StatefulBuilder(
                        builder: (context, setDialogState) => ListTile(
                          ...
                          subtitle: Text(horaInicio?.format(context) ?? 'No definida'),
                          onTap: () async {
                    ...
                    Expanded(
                      child: StatefulBuilder(
                        builder: (context, setDialogState) => ListTile(
                          ...
                          subtitle: Text(horaFin?.format(context) ?? 'No definida'),
                          onTap: () async {
                            final hora = await showTimePicker(
                              context: context,
            ...
            onPressed: () async {
              ...
              final actaData = {
                ...
                'horaInicio': horaInicio != null ? 
                    '${horaInicio!.hour.toString().padLeft(2, '0')}:${horaInicio!.minute.toString().padLeft(2, '0')}:00' : null,
                'horaFin': horaFin != null ? 
                    '${horaFin!.hour.toString().padLeft(2, '0')}:${horaFin!.minute.toString().padLeft(2, '0')}:00' : null,
                'lugar': lugarController.text.trim().isEmpty ? null : lugarController.text.trim(),
                'descripcion': descripcionController.text.trim(),
                'asistentes': asistentesController.text.trim().isEmpty ? null : asistentesController.text.trim(),
                'acuerdos': acuerdosController.text.trim().isEmpty ? null : acuerdosController.text.trim(),
                'proximosCompromiso': compromisosController.text.trim().isEmpty ? null : compromisosController.text.trim(),
                'estado': 'borrador',
              };

              try {
                final response = isEditing
                    ? await ApiService.actualizarActaReunion(acta['id'], actaData)
                ...
                if (response.success) {
                  ...
                  _cargarActas();
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text(isEditing ? 'Acta actualizada exitosamente' : 'Acta creada exitosamente')),
                  );
                } else {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text(response.message ?? 'Error al guardar acta')),
                  );
                }
              } catch (e) {
                ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _cambiarEstado(Map<String, dynamic> acta, String nuevoEstado) async {
    try {
      final response = await ApiService.cambiarEstadoActa(acta['id'], nuevoEstado);
      ...
      if (response.success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(response.message ?? 'Error al cambiar estado')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _eliminarActa(Map<String, dynamic> acta) async {
    final confirmacion = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('¿Estás seguro de que deseas eliminar el acta "${acta['titulo']}"?'),
        actions: [
          TextButton(
    ...
    if (confirmacion == true) {
      try {
        ...
        if (response.success) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(response.message ?? 'Error al eliminar acta')),
          );
        }
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
                      ...
                      if (acta['lugar'] != null) ...[
                        ...
                        Row(
                          children: [
                            ...
                            Text(acta['lugar'], style: TextStyle(color: Colors.grey[600])),
                          ],
                        ),
                      ],
                    ],
                    ...
                    Expanded(
                      child: DropdownButtonFormField<String>(
                        ...
                        items: const [
                          ...
                          DropdownMenuItem(value: 'borrador', child: Text('Borradores')),
                          DropdownMenuItem(value: 'publicada', child: Text('Publicadas')),
                          DropdownMenuItem(value: 'archivada', child: Text('Archivadas')),
                        ],
                        onChanged: (value) {
                          if (value != null) {
                            setState(() {
                    ...
                    : _error != null
                        ...
                        : _actas.isEmpty
                            ...
                            : ListView.builder(
                                ...
                                itemBuilder: (context, index) => _buildActaCard(_actas[index]),
                              ),
              ),
            ],
          ),
