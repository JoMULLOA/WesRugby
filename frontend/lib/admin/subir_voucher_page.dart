...
class _SubirVoucherPageState extends State<SubirVoucherPage> {
  ...
  bool _isLoadingInscripciones = false;  // Cambiar a false para evitar loading inicial
  ...
  void _inicializarDatos() {
    ...
    print('📊 Estado inicial - Loading: $_isLoadingInscripciones, Inscripciones: ${_inscripciones.length}');
  ...
  void _inicializarMesCorrespondiente() {
    ...
    _mesCorrespondiente = '${ahora.year}-${ahora.month.toString().padLeft(2, '0')}';
  }
  ...
  Future<void> _cargarInscripciones() async {
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        print('❌ Error en respuesta: ${response.statusCode} - ${response.message}');
        setState(() {
          _errorInscripciones = response.message ?? 'Error cargando inscripciones';
          _isLoadingInscripciones = false;
        });
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: () {
        print('🎨 BUILD - Loading: $_isLoadingInscripciones, Error: $_errorInscripciones, Inscripciones: ${_inscripciones.length}');
  ...
  Widget _buildInscripcionSelector() {
    ...
    return DropdownButtonFormField<String>(
      ...
      items: _inscripciones.map((inscripcion) {
  ...
  Future<void> _subirVoucher() async {
    ...
    try {
      final response = await PagosService.subirVoucherMensualidad(
        ...
        bancoOrigen: _bancoController.text.isNotEmpty ? _bancoController.text : null,
        numeroOperacion: _numeroOperacionController.text.isNotEmpty ? _numeroOperacionController.text : null,
        observacionesApoderado: _observacionesController.text.isNotEmpty ? _observacionesController.text : null,
      ...
      if (response.statusCode == 201) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('¡Voucher subido exitosamente! Recibirá un comprobante por correo.'),
