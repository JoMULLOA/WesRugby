...
class JustificanteService extends ChangeNotifier {
  ...
  List<Map<String, dynamic>> getFilteredJustificantes({
      ...
      filtered =
          filtered.where((j) {
            return j['fechaInasistencia'] is DateTime
                ? j['fechaInasistencia'].isAfter(
                  fechaDesde.subtract(Duration(days: 1)),
                )
                : true;
          }).toList();
    }
    ...
    if (fechaHasta != null) {
      filtered =
          filtered.where((j) {
            return j['fechaInasistencia'] is DateTime
                ? j['fechaInasistencia'].isBefore(
                  fechaHasta.add(Duration(days: 1)),
                )
                : true;
          }).toList();
    }
    ...
    if (busqueda != null && busqueda.isNotEmpty) {
      filtered =
          filtered
              .where(
                (j) =>
                    j['usuario'].toLowerCase().contains(
  ...
  Map<String, int> getDirectivaStatistics() {
    int pendientes =
        _justificantes.where((j) => j['estado'] == 'Pendiente').length;
    int aprobados =
        _justificantes.where((j) => j['estado'] == 'Aprobado').length;
    int rechazados =
        _justificantes.where((j) => j['estado'] == 'Rechazado').length;
