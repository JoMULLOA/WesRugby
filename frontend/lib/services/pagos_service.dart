    ...
    try {
      return Inscripcion(
        ...
        fechaInscripcion: json['fechaInscripcion'] != null 
            ? DateTime.tryParse(json['fechaInscripcion'].toString()) ?? DateTime.now()
            : DateTime.now(),
      );
    } catch (e) {
...
class PagosService {
  ...
  static Future<ApiResponse> obtenerHistorialApoderado({
    ...
    try {
      String endpoint = '/comprobantes-pago/apoderado/historial?limite=$limite&pagina=$pagina';
  ...
  static Future<ApiResponse> reenviarComprobanteElectronico(String comprobanteId) async {
    return ApiService.post('/comprobantes-pago/apoderado/$comprobanteId/reenviar-comprobante', {});
  }

  /// Obtener estadísticas del apoderado
  static Map<String, dynamic> calcularEstadisticasLocales(HistorialPagos historial) {
    ...
    if (comprobantes.isNotEmpty) {
      ...
      final proximaFecha = DateTime(ultimaFecha.year, ultimaFecha.month + 1, ultimaFecha.day);
