...
class UsuarioService extends ChangeNotifier {
  ...
  Future<String?> addUsuario({
    ...
    try {
      ...
      if (response.statusCode == 201 && response.data != null) {
        ...
        String errorMsg = 'Error al crear usuario: ${response.message ?? 'Error desconocido'}';
        print('❌ $errorMsg');
        _setError(errorMsg);
        return null;
      }
  ...
  Future<bool> updateUsuario(String rut, Map<String, dynamic> data) async {
    try {
      ...
      if (data.containsKey('peso')) {
        backendData['Peso'] = data['peso']; // Nota: En el backend es "Peso" con mayúscula
      }
  ...
  List<Map<String, dynamic>> searchUsuarios(String query) {
    ...
    return _usuarios.where((usuario) {
      return usuario['nombre']?.toString().toLowerCase().contains(lowerQuery) == true ||
             usuario['email']?.toString().toLowerCase().contains(lowerQuery) == true ||
             usuario['rut']?.toString().toLowerCase().contains(lowerQuery) == true ||
  ...
  List<Map<String, dynamic>> filterByRol(String rol) {
    ...
    return _usuarios.where((u) => 
      u['rol']?.toString().toLowerCase() == rol.toLowerCase()
    ).toList();
  }
  ...
  List<Map<String, dynamic>> filterByEstado(String estado) {
    ...
    return _usuarios.where((u) => 
      u['estado']?.toString().toLowerCase() == estado.toLowerCase()
    ).toList();
  }
  ...
  Map<String, dynamic>? getUsuarioByRut(String rut) {
    try {
      return _usuarios.firstWhere((u) => 
        u['rut']?.toString().toLowerCase() == rut.toLowerCase()
      );
    } catch (e) {
      return null;
    }
  ...
  String generateUniqueEmail(String nombre, String rut) {
    ...
    String baseEmail = lastName.isNotEmpty 
    ...
    while (_usuarios.any((u) => u['email'] == email)) {
      String basePart = lastName.isNotEmpty 
  ...
  Map<String, dynamic> get estadisticas {
    ...
    int activos = _usuarios.where((u) => 
      u['estado']?.toString().toLowerCase() == 'activo'
    ).length;
    int pendientes = _usuarios.where((u) => 
      u['estado']?.toString().toLowerCase() == 'pendiente activación'
    ).length;
  ...
  String resetPassword(String rut) {
    String newPassword = 'wessex${DateTime.now().millisecondsSinceEpoch.toString().substring(7)}';
    
    // Actualizar localmente - en un entorno real esto debería llamar a un endpoint específico
    int index = _usuarios.indexWhere((u) => u['rut'] == rut);
    if (index != -1) {
      _usuarios[index]['resetToken'] = 'reset_${DateTime.now().millisecondsSinceEpoch}';
